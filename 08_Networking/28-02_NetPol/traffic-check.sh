...
function install_package() {
  ...
  for pod in db01-sts-0 web01-pod; do
...
function traffic_check_from_app() {
  ...
  check_command_status kubectl -n $NS exec app01-pod -- sh -c "nc -w 1 $DB_IP 5432"
  echo
  echo "* Check traffic from app server to web server"
  check_command_status kubectl -n $NS exec app01-pod -- sh -c "nc -w 1 $WEB_IP 80"
}

function traffic_check_from_db() {
  echo
  echo "* Check traffic from database server to app server"
  check_command_status kubectl -n $NS exec db01-sts-0 -- sh -c "nc -w 1 $APP_IP 3000"
  echo
  echo "* Check traffic from database server to web server"
  check_command_status kubectl -n $NS exec db01-sts-0 -- sh -c "nc -w 1 $WEB_IP 80"
}

function traffic_check_from_web() {
  echo
  echo "* Check traffic from web server to app server"
  check_command_status kubectl -n $NS exec web01-pod -- sh -c "nc -w 1 $APP_IP 3000"
  echo
  echo "* Check traffic from web server to database server"
  check_command_status kubectl -n $NS exec web01-pod -- sh -c "nc -w 1 $DB_IP 5432"
}
